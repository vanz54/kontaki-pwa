...
# How to use it
To use Kontaki, since it is not yet deployed online, first download the project's .zip folder and open it with Visual Studio Code. Next, install the required packages for the project to function by running `npm install`. Once this is done, you must edit the `environment.ts` variable `firebase` with your personal Firebase's project configs and you are ready to use the pwa.
